...
Description: Ilmomasiina tapahtumien luomiseen ja ilmottautumiseen. Luo uusi sivu ja lisää sille shortcode [ilmomasiina] luodaksesi käyttöliittymän ilmomasiinalle.
Version: 0.5.1
*/
...
.ohjekentta, .vaihtoehdotkentta {width: 100%;; min-width: 200px;}
  ...
  for ($i = 0 ; $i < 99; $i++) {
    ...
    echo '	Lomakkeen teksti (Pakollinen + pitää yksilöllinen!): <br />';
...
function tapahtuman_ilmotietoja_metabox() {
	...
	echo '<label for="salli_muokkaus">Salli vastauksen muokkaaminen</label><br />';
	
	echo '<input type="checkbox" name="salli_peruutus" id="salli_peruutus" value=1 '.(get_post_meta($post->ID, '_salli_peruutus', false)?'checked ':'').'/>';
	echo '<label for="salli_peruutus">Salli peruutus (vaatii muokkaamisen)</label><br />';
	
  
	echo '<br /><input type="checkbox" name="piilota_ilmolista" id="piilota_ilmolista" value=1 '.(get_post_meta($post->ID, '_piilota_ilmolista', false)?'checked ':'').'/>';
	echo '<label for="piilota_ilmolista">Piilota julkinen nimilista</label><br />';
	
	echo '<input type="checkbox" name="piilota_ilmomaara" id="piilota_ilmomaara" value=1 '.(get_post_meta($post->ID, '_piilota_ilmomaara', false)?'checked ':'').'/>';
	echo '<label for="piilota_ilmomaara">Piilota ilmoittautuneiden määrä</label><br />';
...
function tapahtumaan_ilmonneet_metabox() {
  ...
  $csvdata = str_replace('"', '%8B', $csvdata);
  $csvdata = str_replace("'", '%8B', $csvdata);
...
function tallenna_tapahtuman_metaboksit($post_id, $post) {
  ...
  $tapahtumameta = array();
	...
	$tapahtumameta['_salli_peruutus'] = ($_POST['salli_peruutus'] ? true: false);
	$tapahtumameta['_varasijat'] = ($_POST['varasijat']? true : false) ;
	$tapahtumameta['_piilota_ilmolista'] = ($_POST['piilota_ilmolista']? true : false) ;
	$tapahtumameta['_piilota_ilmomaara'] = ($_POST['piilota_ilmomaara']? true : false) ;
...
function poista_vanhentuneet_ensiilmot($id) {
	$ilmot = get_post_meta( $id, '_yksittainenilmo', false);
  foreach ($ilmot as $key => $ilmo) {
    if ($ilmo['ensiilmo'] != true) continue;
    $ilmoaika = $ilmo['ilmoaika'];
    $aika = current_time('timestamp');
    if ($aika-$ilmoaika > 630) {
      delete_post_meta($id, '_yksittainenilmo', $ilmo);
    }
  }
